...
func _physics_process(delta):
	...
	if _player != null and current_state != ATTACKING and current_state != JUMPING:
	...
	if current_state == TARGETING:
		if abs(position.x - _player.position.x)  >= 70:
			if _player.position.x > position.x:
					moving_direction = 1
	...
	if current_state != ATTACKING and current_state != JUMPING:
		if velocity.x != 0:
			animation = "walk"
		else:
			animation = "idle"
	elif current_state == ATTACKING or current_state == JUMPING:
		animation = "attack"
...
func _on_AttackArea_player_entered(player):
	if current_state == JUMPING:
